...
comfort=rowMeans(cbind(Q4.na, Q8.na, Q12.na),na.rm=TRUE)
...
Q16A_na1 <- dplyr::na_if(survey$Q16A, 6)
Q16A_na2 <- dplyr::na_if(Q16A_na1, 7)
Q16A_na3 <- dplyr::na_if(Q16A_na2, 8)

Q18A_na1 <- dplyr::na_if(survey$Q18A, 6)
Q18A_na2 <- dplyr::na_if(Q18A_na1, 7)
Q18A_na3 <- dplyr::na_if(Q18A_na2, 8)

Q18B_na1 <- dplyr::na_if(survey$Q18B, 6)
Q18B_na2 <- dplyr::na_if(Q18B_na1, 7)
...
#first, exclude don't know's
Q16A_na <- dplyr::na_if(survey$Q16A, 3)
Q16A_na2 <- dplyr::na_if(Q16A_na, 4)
Q16A_na3 <- dplyr::na_if(Q16A_na2, 5)
Q16A_na4 <- dplyr::na_if(Q16A_na3, 6)

Q16B_na <- dplyr::na_if(survey$Q16B, 3)
Q16B_na2 <- dplyr::na_if(Q16B_na, 4)
Q16B_na3 <- dplyr::na_if(Q16B_na2, 5)
Q16B_na4 <- dplyr::na_if(Q16B_na3, 6)

Q18A_na <- dplyr::na_if(survey$Q18A, 3)
Q18A_na2 <- dplyr::na_if(Q18A_na, 4)
Q18A_na3 <- dplyr::na_if(Q18A_na2, 5)
Q18A_na4 <- dplyr::na_if(Q18A_na3, 6)

Q18B_na <- dplyr::na_if(survey$Q18B, 3)
Q18B_na2 <- dplyr::na_if(Q18B_na, 4)
Q18B_na3 <- dplyr::na_if(Q18B_na2, 5)
Q18B_na4 <- dplyr::na_if(Q18B_na3, 6)

#then, compile columns for turning into categoricals
Q16_bi=rowSums(cbind(Q16A_na4,Q16B_na4), na.rm=TRUE)
Q18_bi=rowSums(cbind(Q18A_na4,Q18B_na4), na.rm=TRUE)
...
### Cross tab of both DV2 questions #####

tradeoff <- table(survey$Q16, survey$Q18)
prop.table(tradeoff, 1) #rows sum to 100%
prop.table(tradeoff, 2) #columns sum to 100%
...
survey2 <-cbind(age_full, gender_full, political_full, religious_full, 
                uni_full, race_full, familiar_gm, familiar_ge,
                familiar_gd, natural, comfort, similar, income,
                trustmean, toxicmean, techmean, globalmean, 
                climate1mean, climate2mean, green1mean, green2mean)
...
####Stepwise regression on DV2####

#some data cleaning (delete/change?)
climatechange <- climate.fa$scores 
technology <- tech.fa$scores 
greenrevolution <- green.fa$scores
globalization <- global.fa$scores
trusts <- trust.fa$scores
toxic <- toxics.fa$scores

###Regressions on ordinal variables for TRADEOFF questions
pestdv <- Q16B_na3 #full sample: survey$Q16
pestdv <- factor(pestdv, ordered=TRUE)
biodivdv <- Q18B_na3 #full sample: survey$Q18
biodivdv <- factor(biodivdv, ordered=TRUE)
pestdata <- data.frame(pestdv, gender_full, uni_full, race_full, age_full, 
                       religious_full, political_full, income, trustmean, toxicmean, 
                       techmean, globalmean, climate1mean, climate2mean, green1mean, green2mean)
biodivdata <- data.frame(biodivdv, gender_full, uni_full, race_full, age_full, 
                         religious_full, political_full, income, trustmean, toxicmean, 
                         techmean, globalmean, climate1mean, climate2mean, green1mean, green2mean)
...
##Pesticide tradeoff stepwise regressions
#backward selection (pesticide data)
pest_all <- glm(pestdv ~ ., data=pestdata, binomial(link="logit"))
step(pest_all, direction = "backward")

#forward selection (pesticide)
pest_start <- glm(pestdv ~ 1, data=pestdata, binomial(link="logit"))
step(pest_start, direction="forward", scope=formula(pest_all))

#stepwise selection (pesticide)
pest_start <- glm(pestdv ~ 1, data=pestdata, binomial(link="logit"))
step(pest_start, direction="both", scope=formula(pest_all))

#based on stepwise selection, what is best model?
pest_step <- glm(formula = pestdv ~ trustmean + techmean + age_full + 
                   trustmean + toxicmean + green2mean + green1mean,
...
##Biodiversity tradeoff stepwise regressions
#backward selection (biodiv data)
biodiv_all <- glm(biodivdv ~ ., data=biodivdata, family = binomial(link="logit"))
step(biodiv_all, direction = "backward")

#forward selection (biodiv)
biodiv_start <- glm(biodivdv ~ 1, data=biodivdata, family = binomial(link="logit"))
step(biodiv_start, direction="forward", scope=formula(biodiv_all))

#stepwise selection (biodiv)
biodiv_start <- glm(biodivdv ~ 1, data=biodivdata, binomial(link="logit"))
step(biodiv_start, direction="both", scope=formula(biodiv_all))

#based on stepwise selection, what is best model?
biodiv_step <- glm(formula = biodivdv ~ climate2mean + trustmean + techmean
                   + religious_full + green2mean,
...
pestdv_bi <- Q16_all_bi
pestdv_bi <- factor(pestdv_bi, ordered=TRUE)
biodivdv_bi <- Q18_all_bi
biodivdv_bi <- factor(biodivdv_bi, ordered=TRUE)
pestdata_bi <- data.frame(pestdv_bi, gender_full, uni_full, race_full, age_full, 
                          religious_full, political_full, income, trustmean, toxicmean, 
                          techmean, globalmean, climate1mean, climate2mean, green1mean, green2mean)
biodivdata_bi <- data.frame(biodivdv_bi, gender_full, uni_full, race_full, age_full, 
                            religious_full, political_full, income, trustmean, toxicmean, 
                            techmean, globalmean, climate1mean, climate2mean, green1mean, green2mean)
...
#backward selection (pesticide data)
require(nnet)
pest_all_bi <- glm(pestdv_bi ~., family = binomial, data=pestdata_bi)
step(pest_all_bi, direction = "backward")

#forward selection (pesticide)
pest_start_bi <- glm(pestdv_bi ~ 1, family = binomial, data=pestdata_bi)
step(pest_start_bi, direction="forward", scope=formula(pest_all_bi))

#stepwise selection (pesticide)
pest_start_bi <- glm(pestdv_bi ~ 1, data=pestdata_bi)
step(pest_start_bi, direction="both", scope=formula(pest_all_bi))

#based on stepwise selection, what is best model?
pest_step_bi <- glm(pestdv_bi ~ trustmean + techmean + gender_full + 
                      age_full + green2mean + green1mean,
...
#backward selection (biodiv data)
require(nnet)
biodiv_all_bi <- glm(biodivdv_bi ~., family = binomial, data=biodivdata_bi)
step(biodiv_all_bi, direction = "backward")

#forward selection (biodiv)
biodiv_start_bi <- glm(biodivdv_bi ~ 1, family = binomial, data=biodivdata_bi)
step(biodiv_start_bi, direction="forward", scope=formula(biodiv_all_bi))

#stepwise selection (biodiv)
biodiv_start_bi <- glm(biodivdv_bi ~ 1, data=biodivdata_bi)
step(biodiv_start_bi, direction="both", scope=formula(biodiv_all_bi))

#based on stepwise selection, what is best model?
biodiv_step_bi <- glm(pestdv_bi ~ trustmean + techmean + gender_full +
                    age_full + green2mean + income + green1mean + political_full + climate2mean,
                    family = binomial, data = pestdata_bi)
...
tomatodata <- data.frame(tomatodv, gender_full, uni_full, race_full, age_full, 
                         religious_full, political_full, income, trustmean, toxicmean, 
                         techmean, globalmean, climate1mean, climate2mean, green1mean, green2mean)
cattledata <- data.frame(cattledv, gender_full, uni_full, race_full, age_full, 
                         religious_full, political_full, income, trustmean, toxicmean, 
                         techmean, globalmean, climate1mean, climate2mean, green1mean, green2mean)
wheatdata <- data.frame(wheatdv, gender_full, uni_full, race_full, age_full, 
                        religious_full, political_full, income, trustmean, toxicmean, 
                        techmean, globalmean, climate1mean, climate2mean, green1mean, green2mean)
...
##Tomato dv
#backward selection (tomato data)
require(nnet)
tomato_all <- glm(tomatodv ~., data=tomatodata, binomial(link="logit"))
step(tomato_all, direction = "backward")

#forward selection (tomato)
tomato_start <- glm(tomatodv ~ 1, data=tomatodata, binomial(link="logit"))
step(tomato_start, direction="forward", scope=formula(tomato_all))

#stepwise selection (tomato)
tomato_start <- glm(tomatodv ~ 1, data=tomatodata, binomial(link="logit"))
step(tomato_start, direction="both", scope=formula(tomato_all))

#based on stepwise selection, what is best model?
tomato_step <- glm(formula = tomatodv ~ green2mean + trustmean + techmean +
                     toxicmean + political_full + religious_full,
...
##cattle dv
#backward selection (cattle data)
require(nnet)
cattle_all <- glm(cattledv ~., data=cattledata, binomial(link="logit"))
step(cattle_all, direction = "backward")

#forward selection (cattle)
cattle_start <- glm(cattledv ~ 1, data=cattledata, binomial(link="logit"))
step(cattle_start, direction="forward", scope=formula(cattle_all))

#stepwise selection (cattle)
cattle_start <- glm(cattledv ~ 1, data=cattledata, binomial(link="logit"))
step(cattle_start, direction="both", scope=formula(cattle_all))

#based on stepwise selection, what is best model?
cattle_step <- glm(formula = cattledv ~ trustmean + green2mean + gender_full +
                     techmean + political_full + toxicmean + uni_full + religious_full,
...
##wheat dv
#backward selection (wheat data)
require(nnet)
wheat_all <- glm(wheatdv ~., data=wheatdata, binomial(link="logit"))
step(wheat_all, direction = "backward")

#forward selection (wheat)
wheat_start <- glm(wheatdv ~ 1, data=wheatdata, binomial(link="logit"))
step(wheat_start, direction="forward", scope=formula(wheat_all))

#stepwise selection (wheat)
wheat_start <- glm(wheatdv ~ 1, data=wheatdata, binomial(link="logit"))
step(wheat_start, direction="both", scope=formula(wheat_all))

#based on stepwise selection, what is best model?
wheat_step <- glm(formula = wheatdv ~ trustmean + techmean + green2mean +
                    age_full + uni_full + political_full + green1mean,
